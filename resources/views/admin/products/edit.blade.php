    ...
    */
        ...
        if (!is_array($bootPrice)) {
            ...
            foreach ($product->prices as $row) {
                // requires $product->prices relation
                $ciKey = is_null($row->color_id) ? 'na' : $colorIndexById[$row->color_id] ?? null;
                $siKey = is_null($row->size_id) ? 'na' : $sizeIndexById[$row->size_id] ?? null;
...
============================================================================ --}}
    <script>
        window.productEditor = function(init) {
            ...
            return {
                    ...
                    colorKeys.forEach(ck => {
                        ...
                        sizeKeys.forEach(sk => {
                            if (!this.prices[ck][sk] || typeof this.prices[ck][sk] !== 'object') {
                                ...
                                this.prices[ck][sk].discounted_price = this.prices[ck][sk]
                                    .discounted_price ?? '';
                            }
                        });
                    });
                 ...
                 * ------------------------- */
                hasColors() {
                    return (this.colors?.length || 0) > 0
                },
                hasSizes() {
                    return (this.sizes?.length || 0) > 0
                },
                matrixCount() {
                ...
                rowTotal(si) {
                    ...
                    let t = 0;
                    for (let ci = 0; ci < this.colors.length; ci++) t += this.getQty(ci, si);
                    return t;
                },
                columnTotal(ci) {
                    if (!this.hasSizes()) return this.getQty(ci, null);
                    let t = 0;
                    for (let si = 0; si < this.sizes.length; si++) t += this.getQty(ci, si);
                    return t;
                },
                grandTotal() {
                    if (this.hasColors() && this.hasSizes()) {
                        let t = 0;
                        for (let si = 0; si < this.sizes.length; si++) t += this.rowTotal(si);
                        return t;
                    }
                    if (this.hasColors()) {
                        let t = 0;
                        for (let ci = 0; ci < this.colors.length; ci++) t += this.columnTotal(ci);
                        return t;
                    }
                    if (this.hasSizes()) {
                        let t = 0;
                ...
                setPrice(ci, si, val) {
                    ...
                    if (!this.prices[ck][sk]) this.prices[ck][sk] = {
                        price: '',
                        discounted_price: ''
                    };
                ...
                dt: new DataTransfer(),
                /* -------------------------
                 * I) SIZE CHART (single image)
                 * ------------------------- */
                sizeChartPreview: init.sizeChartUrl || '',
                sizeChartExisting: init.sizeChartUrl || '',
                sizeChartName: init.sizeChartName || '',
                removeSizeChartFlag: false,

                handleSizeChart(fileList) {
                    ...
                    this.sizeChartName = file.name;
                    this.removeSizeChartFlag = false;

                    const dt = new DataTransfer();
                    dt.items.add(file);
                    this.$refs.sizeChartInput.files = dt.files;

                    const reader = new FileReader();
                    reader.onload = (e) => {
                        this.sizeChartPreview = e.target.result;
                    };
                ...
                removeSizeChart() {
                    ...
                    this.sizeChartName = '';
                    this.removeSizeChartFlag = true;

                    if (this.$refs.sizeChartInput) {
                        this.$refs.sizeChartInput.value = '';
                    }
                },
    ...
    'sizeChartUrl' => $product->size_chart ? asset('storage/' . $product->size_chart) : '',
    'sizeChartName' => $product->size_chart ? basename($product->size_chart) : '',
]))" x-init="$nextTick(() => {
                initStock(@js($bootStock));
                initPrices(@js($bootPrice));
                ...
                <div class="mt-10">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex items-center gap-2 text-sm">
                            ...
                            <button type="button" class="px-3 py-1.5 rounded-md bg-black text-white hover:bg-black/90"
                                @click="const v = promptMoney('Set all prices'); if(v !== null) setAllPrices(v)">Set all
                                Price</button>

                            <button type="button" class="px-3 py-1.5 rounded-md bg-black text-white hover:bg-black/90"
                                @click="const v = promptMoney('Set all discounted'); if(v !== null) setAllDiscounts(v)">Set
                                all Discount</button>
                    ...
                    <p class="text-xs text-gray-500 mb-4">
                        Fill variant price/discount here. Leave blank to fall back to the global Price/Discount inputs
                        above.
                    </p>
                                        ...
                                        <template x-for="(c,ci) in colors" :key="'phead-' + c.name + c.hex + ci">
                                            <th class="p-2 text-left align-bottom min-w-[240px]">
                                                ...
                                                <div class="mt-2 flex items-center gap-2 text-xs flex-wrap">
                                                    <button type="button" class="px-2 py-0.5 rounded border"
                                                        @click="const v=promptMoney('Set column price'); if(v !== null) setColPrice(ci, v)">Set
                                                        col Price</button>
                                                    <button type="button" class="px-2 py-0.5 rounded border"
                                                        @click="const v=promptMoney('Set column discount'); if(v !== null) setColDiscount(ci, v)">Set
                                                        col Discount</button>
                                                </div>
                                            </th>
                                        </template>
                                    ...
                                    <template x-for="(s,si) in sizes" :key="'prow-' + s + si">
                                        <tr class="border-t">
                                            <th class="p-2 align-top">
                                                ...
                                                <div class="mt-2 flex items-center gap-2 text-xs flex-wrap">
                                                    <button type="button" class="px-2 py-0.5 rounded border"
                                                        @click="const v=promptMoney('Set row price'); if(v !== null) setRowPrice(si, v)">Set
                                                        row Price</button>
                                                    <button type="button" class="px-2 py-0.5 rounded border"
                                                        @click="const v=promptMoney('Set row discount'); if(v !== null) setRowDiscount(si, v)">Set
                                                        row Discount</button>
                                                </div>
                                            </th>
                                                    ...
                                                    <div class="grid grid-cols-2 gap-2 min-w-[220px]">
                                                        <div class="relative">
                                                            <input type="number" step="0.01" min="0"
                                                                ...
                                                                placeholder="Price" :value="getPrice(ci, si)"
                                                        ...
                                                        <div class="relative">
                                                            <input type="number" step="0.01" min="0"
                                                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:ring-black focus:border-black"
                                                                placeholder="Discount" :value="getDiscount(ci, si)"
                                    ...
                                    <div class="grid grid-cols-2 gap-2 w-[320px] max-w-full">
                                        <div class="relative">
                                            <input type="number" step="0.01" min="0"
                                                ...
                                                placeholder="Price" :value="getPrice(ci, null)"
                                                @input="setPrice(ci, null, $event.target.value)">
                                            <button type="button"
                                                class="absolute right-1 top-1 text-xs text-gray-400 hover:text-red-600"
                                                @click="setPrice(ci, null, '')">×</button>
                                            <input type="hidden" :name="`price_matrix[${key(ci)}][na][price]`"
                                        ...
                                        <div class="relative">
                                            <input type="number" step="0.01" min="0"
                                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                                                placeholder="Discount" :value="getDiscount(ci, null)"
                                                @input="setDiscount(ci, null, $event.target.value)">
                                            <button type="button"
                                                class="absolute right-1 top-1 text-xs text-gray-400 hover:text-red-600"
                                                @click="setDiscount(ci, null, '')">×</button>
                                            <input type="hidden" :name="`price_matrix[${key(ci)}][na][discounted_price]`"
                                ...
                                <div class="flex items-center justify-between border rounded-md p-2 gap-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 w-[320px] max-w-full">
                                        <div class="relative">
                                            <input type="number" step="0.01" min="0"
                                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                                                placeholder="Price" :value="getPrice(null, si)"
                                                @input="setPrice(null, si, $event.target.value)">
                                            <button type="button"
                                                class="absolute right-1 top-1 text-xs text-gray-400 hover:text-red-600"
                                                @click="setPrice(null, si, '')">×</button>
                                            <input type="hidden" :name="`price_matrix[na][${key(si)}][price]`"
                                        ...
                                        <div class="relative">
                                            <input type="number" step="0.01" min="0"
                                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                                                placeholder="Discount" :value="getDiscount(null, si)"
                                                @input="setDiscount(null, si, $event.target.value)">
                                            <button type="button"
                                                class="absolute right-1 top-1 text-xs text-gray-400 hover:text-red-600"
                                                @click="setDiscount(null, si, '')">×</button>
                                            <input type="hidden" :name="`price_matrix[na][${key(si)}][discounted_price]`"
                ...
                <div>
                    ...
                    <input type="file" name="size_chart" accept="image/*" x-ref="sizeChartInput"
                        @change="handleSizeChart($event.target.files)" class="hidden">

                    <input type="hidden" name="remove_size_chart" :value="removeSizeChartFlag ? 1 : 0">

                    <template x-if="!sizeChartPreview">
                        <button type="button" @click="$refs.sizeChartInput.click()"
                            class="group relative inline-flex items-center gap-2 px-5 py-3 rounded-xl
                   bg-gradient-to-b from-zinc-900 via-black to-zinc-950 text-white font-medium
                   shadow-[0_14px_34px_rgba(0,0,0,0.22)]
                   hover:shadow-[0_18px_40px_rgba(0,0,0,0.28)]
                   hover:scale-[1.02] active:scale-[0.99]
                   transition-all duration-200
                   ring-1 ring-white/10">
                            <span class="material-icons text-[20px]">straighten</span>
                            <span
                                x-text="sizeChartExisting && !removeSizeChartFlag ? 'Replace Size Chart' : 'Import Size Chart'"></span>
                        </button>
                    </template>

                    <template x-if="sizeChartPreview">
                        <div
                            class="mt-4 max-w-md rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
                            <div class="relative">
                                <img :src="sizeChartPreview" alt="Size chart preview" class="w-full h-auto object-cover">

                                <button type="button" @click="removeSizeChart()"
                                    class="absolute top-3 right-3 inline-flex items-center justify-center w-9 h-9 rounded-full
                           bg-white/90 dark:bg-gray-900/80 text-red-600 shadow hover:scale-105 transition">
                                    <span class="material-icons text-[18px]">delete</span>
                                </button>
                            </div>
                            ...
                            <div class="px-4 py-3 flex items-center justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="text-sm font-medium text-gray-800 dark:text-gray-100 truncate"
                                        x-text="sizeChartName || 'Current size chart'"></p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Single image only</p>
                                </div>

                                <button type="button" @click="$refs.sizeChartInput.click()"
                                    class="shrink-0 px-3 py-2 text-sm rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                                    Change
                                </button>
                            </div>
                        </div>
