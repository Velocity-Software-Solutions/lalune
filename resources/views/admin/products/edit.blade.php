...
@section('content')
    @php
        ...
        $existingImages = $product->images
            ->map(function ($img) {
                return [
                    ...
                    'isThumb' => (bool) $img->thumbnail, // <-- here
                ];
            })
            ->values()
            ->all();
        ...
        // existing thumb id
        $existingThumbId = optional($product->images->firstWhere('thumbnail', true))->id; // <-- here

        $bootStock = old('stock_matrix');

        if (!is_array($bootStock)) {
            // Map current product colors/sizes to their indices (as rendered in UI)
            // These use the same order as $product->colors/$product->sizes shown above.
            $colorIndexById = [];
            foreach ($product->colors as $idx => $c) {
                $colorIndexById[$c->id] = (string) $idx; // keys must be strings to match Alpine usage
            }
            ...
            foreach ($product->stock as $row) {
                // Translate FK ids to UI indices; 'na' if that axis is missing (NULL)
                ...
                // If we can't map (e.g., removed option), skip gracefully
...
<template x-for="(img, idx) in imagesNew" :key="'new_' + img.uid">
    <div class="relative group border rounded-lg overflow-hidden bg-white dark:bg-gray-800">
        <!-- keep the hidden UID *inside* the single root -->
        ...
        <div class="p-3 space-y-2">
            ...
            <!-- align color hexes by new file index -->
