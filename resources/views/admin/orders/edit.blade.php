    ...
    $order->items
        ...
        <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-gray-900 rounded-2xl p-5 ring-1 ring-gray-200 dark:ring-gray-700">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" class="text-sm px-3 py-1 rounded bg-gray-100 hover:bg-gray-200"
            ...
            @php
                ...
                $discPromo =
                    collect($promosApplied)->first(function ($p) {
            ...
            {{-- Promotions --}}
            <div
                class="bg-white dark:bg-gray-900 rounded-2xl shadow ring-1 ring-gray-200 dark:ring-gray-700 overflow-hidden">
                ...
                <div class="px-5 py-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="promos[shipping][code]" value="{{ $shipCode }}"
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="block text-xs text-gray-500">Code</label>
                        <input type="text" name="promos[discount][code]" value="{{ $discCode }}"
                            class="form-input w-full" placeholder="e.g. SAVE10">

                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
                            <div>
                                <label class="block text-xs text-gray-500">Type</label>
                                <select name="promos[discount][type]" class="form-select w-full" x-data
                                    x-init="$el.value = '{{ $discType }}'">
                            ...
                            <div>
                                ...
                                <input type="number" min="0" step="0.01" name="promos[discount][percent]"
                                    value="{{ $discPct }}" class="form-input w-full" placeholder="e.g. 10">
                            </div>

                            <div>
                                <label class="block text-xs text-gray-500">Amount (cents, if fixed)</label>
                                <input type="number" min="0" step="1"
                                    name="promos[discount][amount_cents]" value="{{ $discAmtCts }}"
                    ...
                    <div class="px-5 pb-4 text-xs text-gray-500">
                        ...
                        @if ($discType)
                            <span class="inline-flex items-center gap-1">
                                ...
                                @if ($discAmtCts > 0)
                                    <span class="text-gray-400">· − {{ $order->currency ?? 'USD' }}
                                        {{ $fmt($discAmtCts) }}</span>
                                @endif
                            </span>
                        @endif
                ...
                <div class="overflow-x-auto" x-init="$nextTick(() => recalcAll())">
                    <table class="min-w-full text-sm divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-gray-900">
                            @foreach ($order->items as $i => $item)
                                @php
                                    ...
                                    // Try common snapshot keys for image/color/size
                                    $thumb =
                                        $snap['image_url'] ?? ($snap['image'] ?? data_get($snap, 'variant.image_url'));

                                    $colorHex =
                                        $snap['color_code'] ?? ($snap['hex'] ?? data_get($snap, 'variant.color_code'));

                                    // normalize HEX -> "#ABCDEF"
                                    if (
                                        $colorHex &&
                                        !Str::startsWith($colorHex, '#') &&
                                        preg_match('/^[0-9A-Fa-f]{6}$/', $colorHex)
                                    ) {
                                        $colorHex = '#' . $colorHex;
                                    }
                                        ...
                                        ($snap['colorLabel'] ??
                                            ($snap['color'] ??
                                                (data_get($snap, 'variant.color_name') ??
                                                    data_get($snap, 'variant.color'))));

                                    $size = $snap['size'] ?? data_get($snap, 'variant.size');
                                @endphp

                                <tr class="text-gray-900 dark:text-gray-100 align-top">
                                    {{-- Product --}}
                                    <td class="px-5 py-4">
                                        ...
                                        <div class="flex items-start gap-3">
                                            @if ($thumb)
                                                <img src="{{ $thumb }}"
                                                    class="w-12 h-12 rounded object-cover ring-1 ring-gray-200 dark:ring-gray-700"
                                                    alt="">
                                            @endif
                                            <div>
                                        ...
                                        <div class="text-xs text-gray-700 dark:text-gray-300 space-y-1.5">
                                            ...
                                            <div class="flex items-center gap-3">
                                                <span class="inline-flex items-center gap-1.5">
                                                    <span class="inline-block w-4 h-4 rounded-full ring-1 ring-gray-300"
                                                        style="background: {{ $colorHex ?: '#f3f4f6' }}"></span>
                                                    <span class="font-medium">
                                                        {{ $colorName ?: ($colorHex ? strtoupper($colorHex) : '—') }}
                                                    </span>
                                                </span>

                                                @if ($size)
                                                    <span
                                                        class="px-2 py-0.5 rounded border border-gray-300 bg-gray-50 text-gray-800">
                                                        {{ strtoupper($size) }}
                                                    </span>
                                                @endif
                                            </div>

                                            {{-- Tiny edit inputs (optional) --}}
                                            <div class="flex items-center gap-2">
                                                <label class="text-[11px] text-gray-500">Color</label>
                                                <input type="text" name="items[{{ $i }}][color]"
                                                    value="{{ old("items.$i.color", $colorName ?: $colorHex) }}"
                                                    class="form-input form-input-sm w-28">
                                                <label class="text-[11px] text-gray-500 ml-2">Size</label>
                                                <input type="text" name="items[{{ $i }}][size]"
                                                    value="{{ old("items.$i.size", $size) }}"
                                                    class="form-input form-input-sm w-20">
                                            </div>
                                        </div>
                                    </td>
                        ...
                        <tfoot class="bg-gray-50 dark:bg-gray-800 text-sm">
                            <tr>
                                <td colspan="4" class="px-5 py-3 text-right text-gray-600">Items Total</td>
