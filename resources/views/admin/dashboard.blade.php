...
@section('content')
    ...
    <div class="px-4 py-6 space-y-6 overflow-scroll custom-scrollbar scrollbar-hide">
        ...
        @php
            $heroBg =
                $indexData && $indexData->background_image
                    ? \Illuminate\Support\Facades\Storage::url($indexData->background_image)
                    : asset('images/index-hero.jpg');
            $sizeChart =
                $indexData && $indexData->size_chart
                    ? \Illuminate\Support\Facades\Storage::url($indexData->size_chart)
                    : asset('images/size-chart.jpeg');
        @endphp
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4">
            <div class="flex items-start justify-between gap-4">
                <div class="flex-1 space-y-3">
                    ...
                    <form action="{{ route('admin.general.index-hero.update') }}" method="POST"
                        enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
                        @csrf

                        {{-- Editor --}}
                        <div class="lg:col-span-2 space-y-2">
                            <textarea name="content"
                                class="summernote-editor w-full rounded-md border border-gray-300 focus:border-gray-400 focus:ring-1 focus:ring-gray-300">{{ old('content', $indexData?->content) }}</textarea>
                            <p class="mt-1 text-xs text-gray-500">
                                If filled, this HTML replaces the default Designed in Canada / love &amp; care” block.
                            </p>
                        </div>
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <div
                                    class="mt-1 h-24 w-full rounded-lg border border-dashed border-gray-300 bg-gray-50 overflow-hidden">
                            ...
                            <div class="space-y-1">
                                ...
                                @if ($indexData && $indexData->background_image)
                                    <p class="text-[11px] text-gray-500 break-all">
                                        Current: <span class="font-mono">{{ $indexData->background_image }}</span>
                                    </p>
                                @endif
                            </div>
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <div
                                    class="mt-1 h-full w-full max-w-80 rounded-lg border border-dashed border-gray-300 bg-gray-50 overflow-hidden">
                                    <img src="{{ $sizeChart }}" alt="">
                                </div>
                            </div>
                            <div class="space-y-1">
                                <input type="file" name="size_chart_image"
                                    class="block w-full text-xs text-gray-700 file:mr-3 file:rounded-md file:border-0 file:bg-gray-900 file:px-3 file:py-1.5 file:text-xs file:font-medium file:text-white hover:file:bg-gray-800" />
                            </div>
                        </div>
                        <div class="pt-2 m-4 flex items-center gap-2 self-end justify-self-end col-span-2">
                            <button type="submit"
                    ...
                    @if ($indexData)
                        <form action="{{ route('admin.general.index-hero.reset') }}" method="POST"
                            ...
                            <button type="submit"
                                ...
                                Reset BG Image and Content to default
                            </button>
                        </form>
                    @endif
                    @if ($indexData)
                        <form action="{{ route('admin.general.size-chart.reset') }}" method="POST"
                            onsubmit="return confirm('Reset size chart to default? This will delete the saved setup.');"
                            class="inline-flex ml-2">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex items-center justify-center rounded-md border border-red-200 bg-white px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-100 focus:ring-offset-1">
                                Reset Size Chart to default
                            </button>
                        </form>
                    @endif
                </div>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
            {{-- Table --}}
            <div x-data="pendingOrdersTable({ orders: @js($pendingOrders ?? []) })" x-init="init()"
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <input x-model.debounce.300ms="q" type="text" placeholder="Search orders, names, emails…"
                        ...
                        <select x-model="perPage" class="px-2 py-2 text-sm bg-white border border-gray-300 rounded">
                            ...
                            <template x-for="o in paginated" :key="o.id">
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 font-medium text-gray-900">
                                        <a href="#" class="underline decoration-gray-300 hover:decoration-gray-500"
                                            x-text="o.order_number"></a>
                                    </td>
                                    <td class="px-4 py-3">
                ...
                <div class="flex items-center justify-between px-4 py-3">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <template x-for="n in totalPages" :key="n">
                            <button @click="go(n)" class="px-3 py-1.5 text-sm rounded"
    ...
    <script>
        ...
        function pendingOrdersTable(initial = {
            orders: []
        }) {
            return {
