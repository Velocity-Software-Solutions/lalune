...
<header x-data="{ mobileOpen:false, stagger(i){ return `transition-delay:${i*60}ms` } }"
        class="top-0 z-50 shadow bg-black m-6 rounded-3xl">
  <div class="flex items-center justify-between px-[5%] py-2 mx-auto max-w-7xl">
    ...
    <nav class="hidden md:flex gap-6 items-center text-gray-100">
      ...
      <div x-data="{ open:false }" class="relative">
        <button @click="open=!open" @keydown.escape.window="open=false" @click.away="open=false"
          ...
          <span>Policies</span>
          <span class="material-icons text-base transition-transform duration-200" :class="open ? 'rotate-180' : ''">expand_more</span>
        </button>

        <div x-show="open" x-transition
             class="absolute right-0 mt-3 w-64 origin-top-right z-50" style="display:none;">
          <div class="rounded-2xl bg-white/95 backdrop-blur-md shadow-2xl ring-1 ring-black/5 overflow-hidden">
            <div class="h-0.5 w-full bg-gradient-to-r from-neutral-900 via-neutral-600 to-neutral-900"></div>
            <div class="py-2">
              <a href="{{ route('return-policy') }}" class="flex items-center gap-3 px-4 py-3 text-gray-800 hover:bg-black/5">
              ...
              <a href="{{ route('privacy-policy') }}" class="flex items-center gap-3 px-4 py-3 text-gray-800 hover:bg-black/5">
                <span class="material-icons text-neutral-700">shield</span>
                <div><div class="font-medium">Privacy Policy</div></div>
              </a>
              <a href="{{ route('terms-conditions') }}" class="flex items-center gap-3 px-4 py-3 text-gray-800 hover:bg-black/5">
                <span class="material-icons text-neutral-700">gavel</span>
                <div><div class="font-medium">Terms &amp; Conditions</div></div>
              </a>
            </div>
            <div class="px-4 py-2 bg-gradient-to-r from-black via-neutral-700 to-black text-white text-xs">Shop with confidence ✨</div>
          </div>
        </div>
      </div>
    ...
    {{-- Mobile: Hamburger --}}
    <button @click="mobileOpen=true" class="md:hidden text-white p-2 rounded-lg focus:ring-2 focus:ring-white/30">
  ...
  <div x-show="mobileOpen" x-transition.opacity
       class="fixed inset-0 z-[60] bg-black/95 text-white md:hidden" style="display:none;"
       @keydown.escape.window="mobileOpen=false">
    <div class="absolute top-0 left-0 right-0 p-5 flex items-center justify-between">
      <img src="{{ asset('images/logo-horizontal.jpg') }}" alt="{{ __('messages.logo_alt') }}" class="h-10">
      <button @click="mobileOpen=false" class="p-2 focus:outline-none focus:ring-2 focus:ring-white/30 rounded-lg">
    ...
    <nav class="pt-24 px-8 space-y-2">
      ...
      <a href="{{ route('cart.index') }}" @click="mobileOpen=false"
        ...
        Cart
        @if (session('cart'))
          <span class="ml-2 inline-flex items-center justify-center text-xs bg-white text-black rounded-full px-2 h-5 align-middle">
            {{ count(session('cart')) }}
          </span>
        @endif
      </a>
      ...
      <div class="pt-2"
        ...
        <div class="grid">
          <a href="{{ route('return-policy') }}" @click="mobileOpen=false" class="py-2 text-lg hover:text-white/90">Return Policy</a>
          <a href="{{ route('privacy-policy') }}" @click="mobileOpen=false" class="py-2 text-lg hover:text-white/90">Privacy Policy</a>
          <a href="{{ route('terms-conditions') }}" @click="mobileOpen=false" class="py-2 text-lg hover:text-white/90">Terms &amp; Conditions</a>
        </div>
      </div>
      ...
      <div class="pt-2"
           ...
           :style="stagger(5)">
        <div class="grid">
          <a href="{{ route('dashboard') }}" @click="mobileOpen=false" class="py-2 text-lg hover:text-white/90">{{ __('messages.nav_dashboard') }}</a>
          <form action="{{ route('logout') }}" method="POST" class="py-2">
            @csrf
            <button type="submit" class="text-left w-full text-lg hover:text-white/90">{{ __('messages.nav_logout') }}</button>
          </form>
        </div>
      </div>
      @endauth
